...
function UserDashboard({ timeAgo }) {
    ...
    useEffect(() => {
        ...
        const response = axios.get(`${import.meta.env.VITE_BASE_URL}/${user_id}`)
            .then(response => {
    ...
    useEffect(() => {
        const handleProblemsData = async () => {
            ...
            if (user_id) {
                try {
                    const response = await axios.get(`${import.meta.env.VITE_BASE_URL}/problem/${user_id}`);
    ...
    const handleDeleteReport = async (itemId) => {
        try {
            const res = await axios.delete(`${import.meta.env.VITE_BASE_URL}/problem/delete/${itemId}`);
            // console.log(res);
    ...
    const reviewFunction = async (problemId) => {
        ...
        try {
            const repDetail = await axios.get(`${import.meta.env.VITE_BASE_URL}/problem/getProblemData/${problemId}`)
            const { assignTo_id, _id, user_id } = repDetail.data.data
            const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/credit`, { assignTo_id, user_id, problemId, creditScore,feedback, isSolved})
            const updateStatusData = await axios.put(`${import.meta.env.VITE_BASE_URL}/problem/updateIsSolved/${problemId}`,{isSolved})
            setProblemDatas((prevProblemDatas) => 
