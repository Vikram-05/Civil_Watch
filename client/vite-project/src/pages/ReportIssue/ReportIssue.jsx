...
function ReportIssue() {
    ...
    const useCurrentLocation = useCallback(async () => {
        ...
        try {
            ...
            // Now call AllRep with the confirmed values
    ...
    async function getWardFromCoordinates(latitude, longitude) {
        ...
        if (!cachedGeoJSON) {
            const response = await fetch(`/bbmp_wards.geojson?nocache=${Date.now()}`);
                ...
                <div className="preview_box">
                    ...
                    <div className="btn_con">
                        <button type="submit" className='submit' disabled={submitLoader}>
                            {submitLoader ? <div className="lds-dual-ring"></div> : <>{t('SubmitReport')}</>}
                        </button>
                    </div>
                </div>
            </form>
